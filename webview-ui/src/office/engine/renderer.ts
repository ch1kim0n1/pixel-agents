...
import { getCharacterRenderOffset, getCharacterSprite } from './characters.js'
...
export function renderScene(
  ...
  for (const ch of characters) {
    ...
    const renderOffset = getCharacterRenderOffset(ch)
    // Sitting offset: shift character down when seated so they visually sit in the chair
    const sittingOffset = ch.state === CharacterState.TYPE ? CHARACTER_SITTING_OFFSET_PX : 0
    // Anchor at bottom-center of character — round to integer device pixels
    const drawX = Math.round(offsetX + (ch.x + renderOffset.x) * zoom - cached.width / 2)
    const drawY = Math.round(offsetY + (ch.y + sittingOffset + renderOffset.y) * zoom - cached.height)
...
export function renderBubbles(
  ...
  for (const ch of characters) {
    ...
    const bubbleX = Math.round(offsetX + (ch.x + renderOffset.x) * zoom - cached.width / 2)
    const bubbleY = Math.round(
      offsetY + (ch.y + renderOffset.y + sittingOff - BUBBLE_VERTICAL_OFFSET_PX) * zoom - cached.height - 1 * zoom,
    )
